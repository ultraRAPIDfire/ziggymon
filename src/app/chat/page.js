...
import {
  Send, LogOut, Terminal, Cpu, Sun, Moon, Check, Copy, Edit2, X,
  ShieldAlert, Trash2, Menu, AlertTriangle, Bot, User, Loader2, Radio,
} from "lucide-react";
...
function ConfirmationModal({ isOpen, title, message, onConfirm, onCancel, isCritical }) {
  ...
  return (
    <div
      onClick={onCancel}
      className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-md animate-fadeIn"
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className={`w-full max-w-md rounded-2xl border bg-zinc-900 p-6 shadow-[0_0_50px_rgba(0,0,0,0.5)] transform opacity-0 animate-scaleUp ${
      ...
      >
        ...
        <div className="mt-6 flex justify-end gap-2.5 font-mono text-xs">
          <button
            onClick={onCancel}
            type="button"
            className="px-4 py-2 rounded-lg border border-zinc-800 hover:border-zinc-700 text-zinc-400 hover:text-zinc-200 transition-colors active:scale-95"
          >
            CANCEL_REQ
          </button>
          <button
...
function useRevealedText(text, fresh) {
  const [revealed, setRevealed] = useState(fresh ? "" : text);
...
function AssistantBubble({ text, fresh, isDarkMode, onCopyError, pushToast }) {
  const revealed = useRevealedText(text || "", !!fresh);
  const isRevealing = fresh && revealed.length < (text || "").length;

  const handleCopyMessage = async () => {
    try {
      await navigator.clipboard.writeText(text || "");
      pushToast("Message copied to buffer.", "success");
    } catch (err) {
  ...
  return (
    <div className="flex items-start gap-2.5 max-w-[92%] sm:max-w-3xl">
      <div className={`shrink-0 h-7 w-7 rounded-lg border flex items-center justify-center mt-0.5 ${
        isDarkMode ? "bg-zinc-900 border-zinc-800 text-orange-500" : "bg-white border-zinc-200 text-orange-500"
      }`}>
        <Bot size={14} />
      </div>
      <div className={`rounded-2xl rounded-bl-sm px-4 py-3.5 text-xs sm:text-sm leading-relaxed font-mono border shadow-md relative group/ai min-w-0 flex-1 transition-colors duration-300 ${
        isDarkMode ? "bg-zinc-900/40 border-zinc-800 text-zinc-300 backdrop-blur-md" : "bg-white border-zinc-200 text-zinc-800"
      }`}>
        <div className="text-[9px] uppercase tracking-wider font-bold text-zinc-500 mb-2 select-none border-b border-zinc-800/40 pb-1 flex items-center justify-between gap-1.5">
          <span className="flex items-center gap-1.5">
            EXECUTION_LOG // OUTPUT
            {isRevealing && <span className="h-1 w-1 rounded-full bg-orange-500 animate-glowPulse" />}
          </span>
          <button
            onClick={handleCopyMessage}
            type="button"
            className="opacity-0 group-hover/ai:opacity-100 transition-opacity flex items-center gap-1 text-zinc-500 hover:text-orange-400"
          >
            <Copy size={10} />
          </button>
        </div>
        <div className="space-y-1 text-zinc-200 font-sans sm:font-mono">
          <ChatMarkdown text={revealed} onCopyError={onCopyError} />
          {isRevealing && <span className="inline-block w-1.5 h-3.5 -mb-0.5 bg-orange-500 animate-caretBlink align-middle ml-0.5" />}
        </div>
      </div>
    </div>
  );
...
export default function ChatPage() {
  ...
  const fetchSessions = () => {
    ...
    fetch("/api/sessions")
      ...
      .catch(() => {
        pushToast("Failed to load session registry.");
      })
  ...
  if (status === "loading") {
    return (
      <div className="relative flex h-screen items-center justify-center font-mono bg-zinc-950 text-orange-500 tracking-widest text-xs overflow-hidden">
        ...
        <div className="relative flex flex-col items-center gap-3">
          ...
          <span className="animate-pulse">BOOTING_Z_ENGINE_CORE_INTERFACE...</span>
        </div>
      </div>
    );
  }
  ...
  const handleSelectSession = async (sessionId) => {
    ...
    try {
      ...
      if (error.name !== "AbortError") {
        ...
        pushToast("Failed to retrieve chat logs.");
        setLoading(false);
      }
    }
  };
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", text: aiText || "No readable string payload returned from network.", fresh: true },
      ]);
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", text: "Runtime Error: Failed to connect to core intelligence pipeline.", fresh: true },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  const submitRename = async (sessionId) => {
    ...
    try {
      ...
      pushToast("Failed to save session title.");
    }
  };
    ...
    setModalConfig({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          pushToast("Failed to purge sessions.");
        }
      }
    });
  };
  ...
  const handleCopyError = (msg) => pushToast(msg);

  return (
    <div className={`flex h-screen font-sans overflow-hidden transition-colors duration-500 relative ${
      ...
      <div className="absolute inset-0 bg-grain pointer-events-none z-0 opacity-60" />
      {isDarkMode && (
        <div className="absolute top-0 right-0 h-96 w-96 rounded-full bg-orange-600/[0.05] blur-3xl animate-float pointer-events-none z-0" />
      )}
      ...
      <div className={`fixed lg:static inset-y-0 left-0 w-66 border-r flex flex-col justify-between p-4 z-50 transform lg:transform-none transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] backdrop-blur-2xl ${
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                onClick={() => setIsSidebarOpen(false)}
                type="button"
                className={`p-2 rounded-xl border transition-colors lg:hidden ${isDarkMode ? "border-zinc-800 text-zinc-500 hover:bg-zinc-800" : "border-zinc-300 text-zinc-500 hover:bg-zinc-200"}`}
              >
                <X size={14} />
              </button>
            </div>
          ...
          <div className={`flex items-center gap-1.5 text-[9px] font-mono uppercase tracking-widest mb-5 ${isDarkMode ? "text-zinc-600" : "text-zinc-400"}`}>
            <Radio size={9} className={loading ? "text-orange-500 animate-pulse" : "text-emerald-500"} />
            <span className={loading ? "text-orange-500" : ""}>{loading ? "processing_stream" : "link_established"}</span>
          </div>
          ...
          <div className="flex flex-col gap-1.5 mb-5">
            ...
            >
              ...
              {isTemporaryMode && <span className="h-1.5 w-1.5 rounded-full bg-purple-400 text-purple-400 animate-glowPulse" />}
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-1 pr-1 custom-scrollbar">
            {sessionsLoading && chatSessions.length === 0 ? (
              Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="h-9 rounded-xl bg-zinc-800/30 bg-shimmer animate-shimmer" style={{ animationDelay: `${i * 0.1}s` }} />
              ))
            ) : (
              chatSessions.map((chat) => (
                <div
                  key={chat.id}
                  className={`group flex items-center justify-between p-2 text-xs rounded-xl border font-mono transition-all duration-200 relative overflow-hidden ${
                ...
                >
                  {currentSessionId === chat.session_id && !isTemporaryMode && (
                    <span className="absolute left-0 top-0 bottom-0 w-[3px] bg-orange-500 animate-glowPulse text-orange-500" />
                  )}
                  {editingSessionId === chat.session_id ? (
                    <div className="flex items-center gap-1 w-full" onClick={(e) => e.stopPropagation()}>
                      ...
                      />
                      <button onClick={() => submitRename(chat.session_id)} type="button" className="text-green-500 p-0.5"><Check size={12} /></button>
                      <button onClick={() => setEditingSessionId(null)} type="button" className="text-zinc-500 p-0.5"><X size={12} /></button>
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="flex lg:opacity-0 group-hover:opacity-100 items-center gap-1 transition-all shrink-0">
                        ...
                        <button type="button" onClick={(e) => triggerDeleteSession(e, chat.session_id)} className="p-0.5 text-zinc-500 hover:text-red-400 transition-colors">
                          <Trash2 size={11} />
                        </button>
                      </div>
                    </>
        ...
        <div className={`border-t pt-4 flex items-center justify-between mt-auto ${isDarkMode ? "border-zinc-800/60" : "border-zinc-200"}`}>
          ...
          <button onClick={() => signOut()} type="button" className={`p-2 rounded-xl transition-all active:scale-90 ${isDarkMode ? "text-zinc-500 hover:text-red-400 hover:bg-zinc-800" : "text-zinc-400 hover:text-red-500 hover:bg-zinc-200"}`}>
            <LogOut size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden z-10">
        ...
        <div className={`lg:hidden flex items-center justify-between px-4 py-3 border-b backdrop-blur-md transition-colors ${
          ...
          <div className="w-8 h-8 flex items-center justify-center">
            <Radio size={12} className={loading ? "text-orange-500 animate-pulse" : "text-emerald-500"} />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 max-w-full custom-scrollbar bg-transparent">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto space-y-4 px-4 animate-fadeIn">
              <div className="relative p-4 rounded-2xl bg-zinc-900/20 border border-zinc-800/30 shadow-inner">
                <div className="absolute inset-0 rounded-2xl bg-orange-500/5 blur-xl" />
                <Terminal size={28} className={`relative ${isDarkMode ? "text-zinc-700 animate-pulse" : "text-zinc-400"}`} />
              </div>
          ...
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} max-w-full animate-messageSlide`}>
              {msg.role === "user" ? (
                <div className="flex items-start gap-2.5 max-w-[85%] sm:max-w-2xl">
                  <div className="rounded-2xl rounded-br-sm px-4 py-3 text-xs sm:text-sm leading-relaxed font-sans bg-zinc-800 text-zinc-100 border border-zinc-700/60 shadow-lg whitespace-pre-wrap">
                    {msg.text}
                  </div>
                  <div className="shrink-0 h-7 w-7 rounded-lg border border-zinc-700 bg-zinc-800 text-zinc-300 flex items-center justify-center mt-0.5">
                    <User size={14} />
                  </div>
                </div>
              ) : (
          ...
          {loading && (
            <div className="flex justify-start animate-messageSlide">
              <div className="flex items-start gap-2.5">
                ...
                <div className={`border rounded-2xl rounded-bl-sm px-4 py-3.5 text-xs font-mono tracking-wider flex items-center gap-2.5 ${
                  isDarkMode ? "bg-zinc-900/60 text-orange-500/80 border-zinc-800/80" : "bg-zinc-100 text-zinc-400 border-zinc-200"
                }`}>
                  <span>ROUTING CLOUD GRAPH PIPELINE</span>
                  <span className="flex items-center gap-0.5">
                    <span className="h-1 w-1 rounded-full bg-current animate-dotBounce" style={{ animationDelay: "0s" }} />
                    <span className="h-1 w-1 rounded-full bg-current animate-dotBounce" style={{ animationDelay: "0.15s" }} />
                    <span className="h-1 w-1 rounded-full bg-current animate-dotBounce" style={{ animationDelay: "0.3s" }} />
                  </span>
                </div>
              </div>
            </div>
        ...
        <form onSubmit={handleSendMessage} className={`p-3 sm:p-4 border-t backdrop-blur-md transition-colors ${
          ...
          <div className="max-w-4xl mx-auto flex gap-2 sm:gap-3 items-end">
            <textarea
              ...
              className={`flex-1 border rounded-xl px-4 py-3 text-xs sm:text-sm focus:outline-none font-mono transition-all duration-300 min-w-0 resize-none max-h-40 custom-scrollbar ${
                isTemporaryMode
                  ? "focus:border-purple-500/50 focus:shadow-[0_0_0_3px_rgba(168,85,247,0.1)] bg-zinc-950/80 border-purple-900/30 text-zinc-100 placeholder-purple-900/30 shadow-inner"
                  : isDarkMode
                    ? "bg-zinc-950/80 border-zinc-800/80 text-zinc-100 placeholder-zinc-700 focus:border-orange-500/40 focus:shadow-[0_0_0_3px_rgba(249,115,22,0.1)] focus:bg-zinc-950 shadow-inner"
                    : "bg-white border-zinc-300 text-zinc-900 placeholder-zinc-400 focus:border-orange-500/40 focus:shadow-[0_0_0_3px_rgba(249,115,22,0.08)]"
              }`}
            />
            <button
              type="submit"
